"""Analysis script behind the translation-length-budget constants in cli.py.

Reads the *.measure.log files under data/measurements/ -- runs of `[measure]`
DEBUG lines captured from real pipeline runs. That directory is NOT tracked by
git (.gitignore ignores data/*), so this script only runs where those logs are
present locally; five exist, but only two are parsed: clip-translate-synth and
trailer-translate-synth. Parsing them offline (the default, no flags)
reproduces:
...
The `[measure]` line format this parses is emitted by cli.py and dub.py.
There is no import edge either way and no CI signal, so renaming or reordering
a field there silently breaks this script.

Usage:
    uv run python scripts/measure.py               # free, offline
    uv run python scripts/measure.py --rerun-paid   # + paid ASR/translation re-run
"""
...
_LOG_DIR = _REPO_ROOT / "data" / "measurements"
CLIP_LOG = _LOG_DIR / "clip-translate-synth.measure.log"
TRAILER_LOG = _LOG_DIR / "trailer-translate-synth.measure.log"
