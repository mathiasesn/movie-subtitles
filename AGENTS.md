...
movie_subtitles/
  ...
  dub.py           # scene-anchored TTS synthesis: groups segments by inter-segment
                    ...
                    # ThreadPoolExecutor (--dub-workers); each batch is resolved by
                    # _resolve(), which fails fast -- cancels the still-queued tail on
                    # the first exception and re-raises it -- before assembling the
                    # silent timeline via ffmpeg; boundary measurement itself lives in
...
- **Defaults are duplicated** between argparse and every public callable's signature that shares it, not just class `__init__`s — e.g. the `--dub-workers` default of 8 legitimately exists in argparse, in `create_subtitles()`, and in `dub._MAX_WORKERS`. This is a deliberate choice, not a defect to collapse; change every copy together.
