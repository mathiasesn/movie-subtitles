...
movie_subtitles/
  ...
  separate.py      # Separate: lazy-imports demucs (and torch) only inside __init__/
                    # separate(), loads the htdemucs model once, then separate() extracts
                    # the source audio via ffmpeg.run() at the model's own rate/channels,
                    # runs apply_model, and sums every stem except "vocals" (indexed via
                    # model.sources) into an accompaniment wav for mux.py to mix the dub
                    # over. Demucs works internally at 44.1 kHz stereo, so a 5.1/48 kHz
                    # source is downmixed in the stem -- mux_dub still re-targets the
                    # final output to the source's own format. Raises rather than
                    # catching; cli.py:_dub_and_mux is what degrades on failure. Cleans
                    # up its own temp dir in a finally
...
`demucs` and `torchaudio` are runtime dependencies of `separate.py`'s `--separate-background` path: `demucs` provides the `htdemucs` separation model, `torchaudio` its wav load/save around the Demucs call. Both are declared explicitly in `pyproject.toml` despite `torchaudio` resolving transitively via `demucs`, per this repo's convention of declaring anything imported directly. Both `demucs.pretrained`/`demucs.apply` are imported lazily, inside `Separate.__init__`/`Separate.separate` respectively, so a run without `--separate-background` never pays Demucs's (or torch's) import cost.
