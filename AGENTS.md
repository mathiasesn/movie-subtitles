...
- **Vendor errors are translated to `RuntimeError` at the provider edge.** Every provider that calls a vendor SDK (`providers/elevenlabs.py`, `providers/openai_.py`, `providers/llm.py`, `dubbing.py`) catches its SDK's documented top-level error base -- `elevenlabs.core.api_error.ApiError`, `openai.OpenAIError`, `anthropic.AnthropicError` -- never bare `Exception`, which would mask genuine bugs -- and re-raises `RuntimeError(<actionable message>) from exc`. This keeps `dub.py` free of vendor SDK imports and keeps issue #17's "`_resolve()` re-raises the original exception instance" contract literally true, since by the time `_resolve()` sees the exception it already *is* a `RuntimeError`; it's also why `cli.main()`'s caught tuple did not need widening to handle a vendor 429. Two call sites (`elevenlabs.py:Speak.speak`'s `text_to_speech.convert`, `dubbing.py:ManagedDub`'s `dubbing.audio.get`) return a lazily-streamed response, so the guard must span the iteration/write loop, not just the call, or the request is never actually issued (and never actually caught) until the iterator is consumed; `Speak.speak` also writes to a `.part` file and renames it on success, so a mid-stream failure leaves no truncated clip behind. `providers/pyannote_.py` is excluded (it already translates its own gate/hub failures), as is `providers/local.py` (no vendor HTTP SDK) and `separate.py` (raises by design; `cli.py` degrades it). New backends calling a vendor SDK must follow this shape too.
