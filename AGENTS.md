...
- **Dub synthesis batches fail fast, they don't drain.** Each of `dub.py`'s two synthesis phases submits one flat cross-group batch to the shared `ThreadPoolExecutor` and gets its clips back from `_resolve()`, which is the only path to them -- collecting results without first waiting is not expressible. On the first exception, `_resolve()` cancels every not-yet-started future in that batch (already-running tasks are left to finish) and re-raises the original exception; only TTS calls (3 attempts, 1s/2s backoff) are retried, the aligner call is not.
