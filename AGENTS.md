...
movie_subtitles/
  ...
  cli.py           # argparse entry point + create_subtitles() orchestration, engine
                    ...
                    # expansion ratio and duration-derived slot capacity), top-level
                    # error handling.
  ...
  config.py        # default_config_path() + load_config(path): strict YAML loader for
                    # CLI defaults (issue #14), main()-only -- importing this module
                    # reads no file from disk, matching _load_env()'s posture. Single
                    # per-user file at $XDG_CONFIG_HOME/movie-subtitles/config.yaml
                    # (falling back to ~/.config/...), parsed with yaml.safe_load only.
                    # A missing default file returns {} (a config file is optional,
                    # same as .env); an explicit --config path that doesn't exist raises
                    # ValueError naming it. Every key is validated against a declared
                    # _SCHEMA of argparse dest names -- unknown key, wrong type, or
                    # out-of-choices value all raise ValueError before any work starts,
                    # in the style of voices.py:load_preset_table. A secret-looking key
                    # (elevenlabs_api_key/anthropic_api_key/openai_api_key/hf_token,
                    # case-insensitive) is checked first and gets a tailored error
                    # pointing at .env instead of the generic unknown-key message. Logs
                    # the loaded path at INFO via the flat "config" logger. Deliberately
                    # does not import cli.py (cli.py imports this module instead --
                    # the reverse would be circular), so its three engine-choice tuples
                    # are a second copy of cli.py's argparse choices=, not read off it.
                    # check_parser_coverage(parser) is what keeps that copy honest:
                    # main() calls it after building the parser, and it raises
                    # RuntimeError if _SCHEMA's keys or choices have drifted from the
                    # parser's own -- outside main()'s try/except, since drift is this
                    # repo's bug, not the user's. Also home to is_positive_int/
                    # is_positive_float/is_unit_float, imported BY cli.py's argparse
                    # type= callables so a bound can't diverge between a flag and its
                    # config key
  ...
  dub.py           # scene-anchored TTS synthesis: groups segments by inter-segment
                    ...
                    # fails fast -- sets a per-call threading.Event abort flag, then
                    # cancels the still-queued tail, on the first exception, and
                    # re-raises it -- before assembling the silent timeline via ffmpeg;
                    # boundary measurement itself lives in providers/ffmpeg_align.py,
                    # providers/elevenlabs.py and providers/fallback.py, not here. The
                    # TTS retry (_tts_with_retry) sleeps a full-jitter backoff --
                    # uniform(0, base * 2**(attempt-1)) -- rather than a fixed one, and
                    # checks the abort flag before its vendor call and before each
                    # retry sleep, raising a private _Aborted sentinel so a doomed
                    # batch stops issuing not-yet-started calls (it cannot cancel one
                    # already in flight). Also returns the coalesced, padded (start,
                    # end) speech spans (_speech_spans) it placed clips into, for
                    # mux.py to duck against
  ...
  dubbing.py        # ManagedDub: the --managed path, independent of the rest. Every
                    # vendor call (create/get/audio.get) catches
                    # elevenlabs.core.api_error.ApiError and re-raises
                    # RuntimeError(...) from exc; audio.get's guard spans its
                    # streamed-chunk write loop, same reason as Speak.speak above
  providers/
    ...
    elevenlabs.py      # ScribeTranscribe (ASR) + Speak (TTS) + Align (Forced Alignment,
                       ...
                       # Every vendor call (plus clone_voice/delete_voice) catches
                       # elevenlabs.core.api_error.ApiError and re-raises
                       # RuntimeError(...) from exc; Speak.speak's guard spans the
                       # text_to_speech.convert iteration/write loop, since the SDK
                       # doesn't issue the request until the returned iterator is
                       # consumed, and writes to a .part file, renamed on success, so a
                       # mid-stream failure leaves no truncated clip behind
    ...
    llm.py             # LLMTranslate: Claude-backed TranslationProvider. The
                       # messages.create call catches anthropic.AnthropicError and
                       # re-raises RuntimeError(...) from exc
    openai_.py          # OpenAITranscribe (ASR) + OpenAITranslate + OpenAISpeak (TTS);
                         ...
                         # (two-pointer sweep, both lists already time-ordered). Every
    ...
    errors.py             # vendor_errors(exc_type, what): shared contextmanager
                          # translating a vendor SDK exception into RuntimeError(f"{what}
                          # failed: {exc}"), used at every vendor call site in
                          # elevenlabs.py, openai_.py, llm.py and dubbing.py. Imports no
                          # vendor SDK -- each call site still names its own SDK's
                          # exception type
```

Flat package, no `src/` layout, no `tests/`.

Neither `specs/` nor `data/` is tracked: `.gitignore` ignores `data/*`, and `specs/.gitignore` ignores everything under `specs/`. Design specs and measurement records live there locally only — do not expect them in a fresh clone, and do not cite them from code comments or docs. What *is* committed is the instrumentation they were derived from: `cli.py` and `dub.py` emit permanent `[measure]` DEBUG lines (one per translated segment, one per synthesis attempt) whenever `MOVIE_SUBTITLES_LOG_LEVEL=DEBUG`. Their field names and order are parsed by `scripts/measure.py`, which reproduces the budget constants from those logs — renaming or reordering a field silently breaks it, with no import edge and no CI signal to catch it, which is why both emitters carry an inline comment naming their parser. `scripts/measure.py` is committed, but the `data/measurements/*.measure.log` files it reads are not, so it runs only where those logs are present locally. The measured constants themselves (`cli.py`'s `_EXPANSION_RATIO` and `_TARGET_SPEAKABLE_CPS`) carry their sample description, derivation, and PROVISIONAL caveats in comments at the definition site; those comments are now the authoritative record of how the numbers were obtained.
...
- **Dub synthesis batches fail fast, they don't drain.** `dub.py` no longer has a fixed two-phase structure: an initial pass at 1.0x is followed by a bounded correction loop (`--dub-correction-passes`, default 3), each pass submitting one flat cross-group batch -- only the segments of groups still outside tolerance -- to the shared `ThreadPoolExecutor` and getting its clips back from `_resolve()`, which is the only path to them -- collecting results without first waiting is not expressible. `_resolve()` flattens every batch handed to it before waiting, so on the first exception it sets a per-call `threading.Event` abort flag -- stopping every not-yet-issued TTS call and cutting short every queued retry sleep across the whole pool, not just tasks that hadn't started -- and *then* cancels the still-queued tail, across *all* of those batches, not just the one containing the failure -- this holds per pass/phase, not just for the old fixed phase A/phase B. It then classifies every future in one pass: the five counts (succeeded/failed/aborted/cancelled/in-flight) are mutually exclusive and sum to the batch size, and nothing already finished can be reported as in-flight, but "in-flight" is not a final state -- those tasks may finish before the WARNING is even emitted. A worker that saw the abort flag and bailed out raises a private `_Aborted` sentinel, counted as `aborted` and excluded from the failures considered for re-raise, so an early bail can never be reported as the batch's cause; note the abort flag cannot stop an HTTP request already in flight, only calls not yet issued, so a batch where every task has already called the vendor reports `aborted` as 0. It re-raises the lowest-submission-index failure among the *real* failures observed during that pass, which is deterministic given that set but not a total order over every failure two truly concurrent tasks could raise.
- **Retry is a property of the batch, not of any vendor.** `dub.py` wraps only the TTS call (3 attempts, full-jitter backoff -- `uniform(0, 1.0 * 2**(attempt-1))`, so 1s/2s are window ceilings, not fixed waits) in retry, never the aligner call, and this policy lives in `dub.py` rather than inside a provider precisely because it must hold identically for every TTS backend; only `dub.py` knows that one unretried failure now aborts the whole queued batch. The jitter matters specifically because a fixed backoff let workers that collided on a vendor concurrency cap wake and re-collide in lockstep, unable to ever drain past a standing cap -- jitter decorrelates them instead, though it makes recovery likely, not guaranteed.
- **Vendor errors are translated to `RuntimeError` at the provider edge.** Every provider that calls a vendor SDK (`providers/elevenlabs.py`, `providers/openai_.py`, `providers/llm.py`, `dubbing.py`) wraps the call in `providers/errors.py:vendor_errors(exc_type, what)`, a shared contextmanager that catches its SDK's documented top-level error base -- `elevenlabs.core.api_error.ApiError`, `openai.OpenAIError`, `anthropic.AnthropicError` -- never bare `Exception`, which would mask genuine bugs -- and re-raises `RuntimeError(f"{what} failed: {exc}") from exc`; each call site still names its own SDK's exception type, and `vendor_errors` itself imports no vendor SDK. This keeps `dub.py` free of vendor SDK imports and keeps issue #17's "`_resolve()` re-raises the original exception instance" contract literally true, since by the time `_resolve()` sees the exception it already *is* a `RuntimeError`; it's also why `cli.main()`'s caught tuple did not need widening to handle a vendor 429. Two call sites (`elevenlabs.py:Speak.speak`'s `text_to_speech.convert`, `dubbing.py:ManagedDub`'s `dubbing.audio.get`) return a lazily-streamed response, so the `with vendor_errors(...)` block must span the iteration/write loop, not just the call, or the request is never actually issued (and never actually caught) until the iterator is consumed; both `elevenlabs.py:Speak.speak` and `openai_.py:OpenAISpeak.speak` write to a `.part` file and rename it on success (one `finally: tmp_path.unlink(missing_ok=True)` cleanup site each), so a mid-stream or write failure leaves no truncated clip behind -- for OpenAI this is a durability guarantee, not a streaming-error guard, since its response body is already fully read by the time `create()` returns. `providers/pyannote_.py` is excluded (it already translates its own gate/hub failures), as is `providers/local.py` (no vendor HTTP SDK) and `separate.py` (raises by design; `cli.py` degrades it). New backends calling a vendor SDK must follow this shape too.
...
`pyyaml` is a runtime dependency of `config.py`'s CLI-defaults config-file loader, which parses the per-user YAML file with `yaml.safe_load` only (never the unsafe loader). Imported at module top level, unlike the lazy dependencies above — `config.py` is imported and its loader invoked unconditionally from `cli.main()` on every run, config file present or not, so there is no lazy-import case to preserve here.

## Things that don't exist yet

No tests, no test framework. There are now four environment variables, not three: `ELEVENLABS_API_KEY`/`ANTHROPIC_API_KEY`/`OPENAI_API_KEY` plus `HF_TOKEN` (see `.env.example` at the repo root, which holds placeholders for all four). `HF_TOKEN` is the `huggingface_hub` standard name; `providers/pyannote_.py:Diarize.__init__` reads it explicitly via `os.environ.get("HF_TOKEN")` and passes it as `token=` to `Pipeline.from_pretrained()` — `cli._load_env()` needed no per-key change, since it already calls `load_dotenv` over the whole file with no allowlist. All four are loaded from `.env` by `cli._load_env()`, called from `main()` only — `find_dotenv(usecwd=True)` so discovery is anchored to the user's cwd rather than to the installed package, and already-exported variables are never overridden. Importing any module still reads no `.env`; library callers set the environment themselves. `MOVIE_SUBTITLES_LOG_LEVEL` is a fifth environment variable, but not a secret: it raises the logging level of this package's own loggers above their default INFO (the root logger stays at INFO, so vendor SDK DEBUG output stays off) (e.g. `DEBUG`, to surface `cli.py`/`dub.py`'s permanent `[measure]` instrumentation lines), unset or unrecognised falls back to INFO, and there is deliberately no CLI flag for it. There is now a minimal error-handling layer at the `main()` boundary (`RuntimeError | ValueError | FileNotFoundError | TimeoutError | subprocess.CalledProcessError` caught and turned into a one-line message + `SystemExit(1)`); anything else still propagates as a traceback. If asked to add tests, that means introducing pytest and a `tests/` directory from scratch — don't go looking for existing ones.

There **is** now a general CLI-defaults config file, not just `--voice-preset-table`'s per-key one: a single per-user YAML file (`config.py:default_config_path()`, `$XDG_CONFIG_HOME/movie-subtitles/config.yaml` or `~/.config/movie-subtitles/config.yaml`) whose keys mirror every argparse `dest` except `--input`/`--config` themselves, loaded once by `cli.main()` and applied via `parser.set_defaults(**resolved)` before the real `parse_args()` runs — this is what makes precedence (explicit flag > config file > built-in default) fall out of argparse itself rather than needing sentinel defaults sprinkled through `create_subtitles()`. A `parser.parse_known_args()` pre-pass reads only `--config` before `set_defaults()` is called, since `--config` must be known before the config file it names can be loaded. A missing default-location file is not an error (mirrors `_load_env()`'s posture toward a missing `.env`); an explicit `--config <path>` that doesn't exist raises `ValueError`. `config.py:load_config` validates strictly against a declared schema — a secret-looking key (`elevenlabs_api_key`/`anthropic_api_key`/`openai_api_key`/`hf_token`, case-insensitive) is rejected first with a message pointing at `.env` instead of this file, then an unknown key, then a wrong type/out-of-choices value — all before any work starts. `config.example.yaml` at the repo root documents every key, commented out at its built-in default. Because `_SCHEMA` is a hand-written copy of an option surface no import edge ties it to, `config.py:check_parser_coverage(parser)` is called from `main()` right after the parser is built: it diffs `_SCHEMA`'s keys against the parser's own dests (minus `help`/`input`/`config`) and each flag's `choices=` against the schema's, and raises `RuntimeError` on any mismatch. It sits *outside* `main()`'s try/except deliberately — schema drift is this repo's bug, not the user's, so it should surface as a traceback rather than a tidy one-line message. Add a flag, add its schema entry, or the next run fails loudly instead of the flag silently not being configurable. The numeric bound predicates (`is_positive_int`/`is_positive_float`/`is_unit_float`) live in `config.py` and are imported *by* `cli.py`'s argparse `type=` callables for the same reason: a config key and its flag are one setting, and a divergent bound would silently accept through one door what the other rejects. Config loading sits inside `main()`'s existing top-level try/except, so a `ValueError` here becomes the same one-line message + `SystemExit(1)` as any other config error. The four `store_true` flags (`--dub`, `--managed`, `--separate-background`, `--keep-cloned-voices`) are now `BooleanOptionalAction` instead, i.e. `--no-dub`/`--no-managed`/`--no-separate-background`/`--no-keep-cloned-voices` all exist now too — needed so a single invocation can override a config file that turned one of them on by default; their argparse defaults are still `False`, so a run with no config file is unchanged.

`--voice-preset-table` remains its own, narrower, config-file format: it takes a path to a JSON file (schema: top-level keys are TTS engine names, each mapping `gender:age_band`/`default` profile keys to a voice id string) that replaces `voices.py`'s built-in preset table wholesale for the engines it names. `voices.py:load_preset_table` validates it strictly at load time — malformed JSON, an unrecognised engine or profile key, a non-string voice id, or a missing `default` entry all raise `ValueError` before any dub work starts, rather than surfacing as a mid-dub `KeyError`. The two config files are unrelated: `--voice-preset-table`'s path is itself just one of the many keys `config.py`'s YAML file can set.
