...
**Resolved:** the `--dub` timing-drift strategy now corrects both directions — overruns (audio longer than its slot) and underruns (audio shorter than its slot) both get a clamped rate re-synthesis. See section 5 and README.md "Timing drift".
...
movie_subtitles/
  ...
  dub.py                  # per-segment TTS synthesis + timing-drift rate-fitting loop
                           # (fit_rate, synthesise_track), skipping the clamped-rate retry
                           # for hopelessly mismatched slots (_HOPELESS_RATE), and silent-
...
8. **Timing-drift fitting is a bounded, single strategy, not a pluggable one.** `dub.py:fit_rate(actual_duration, slot_duration) -> float` is pure: it returns only the TTS speaking-rate adjustment — the unclamped exact-fit ratio comes from the shared `_ideal_rate()` helper, so the ratio and its zero-duration guards are computed in exactly one place — clamped to `[0.9, 1.15]` — intentionally tighter than the ElevenLabs API's own documented `voice_settings.speed` range of `0.7-1.2` — and does not itself decide what the clamp means. `_synthesise_fitted` is the caller that acts on it: it synthesises once at rate 1.0, and if the measured duration drifts from the slot by more than roughly 1% (`round(rate, 2) != 1.0`, over **or** under), it re-synthesises once at the clamped rate — both overruns (audio longer than its slot) and underruns (audio shorter than its slot) are corrected this way, not just overruns. Before any of that, though, a hopeless case short-circuits: if the slot duration is non-positive, or the unclamped ideal rate exceeds `_HOPELESS_RATE = 2.0` (the clip is more than twice its slot), a second TTS call is skipped entirely — a ±15% clamped nudge is noise against a mismatch that large — and the clip is placed unfitted at 1.0x, logged at warning level, and reported as an overrun without ever being re-synthesised or re-measured. So a run makes up to two paid TTS calls per segment, not one — previously only overrunning segments paid twice, underruns now do too — except for hopeless segments, which always cap at one. After a clamped-rate retry, `_synthesise_fitted` re-measures the actual clip with `ffprobe` and returns `(clip_path, still_overruns)`, where `still_overruns` is True only if the clip is still longer than its slot after the clamped retry (an underrun never sets it — a short clip just leaves trailing silence in its slot, which is not itself a problem); for a hopeless segment it is True by construction, not from a measurement. A segment that still overruns is logged and counted (`synthesise_track`'s `overrun_count`, reported as "still overrun their slot after fitting" — the log line deliberately does not say "after the rate clamp", since hopeless segments were never clamped), not force-fit by further distorting speech. There is no `--drift-strategy` flag — see `specs/elevenlabs-port.md` for the alternatives considered and cut (silence trim/pad, Forced-Alignment re-anchoring).
9. **ffmpeg-presence checking is deliberately duplicated, not a bug to deduplicate.** `cli.py:_check_ffmpeg_tools()` and `mux.py:mux_dub` both check for `ffmpeg`/`ffprobe` on `PATH`. This is intentional: `cli.py`'s check is the early, user-facing gate that fails `--dub` before any paid API call is made (ASR/translation/TTS); `mux.py`'s check is a library-level guard for callers that import and invoke `mux_dub` directly, bypassing `cli.py` entirely. Do not remove either check to "deduplicate" them.
...
**Cost:** because underruns are now re-synthesised too, a dub run makes up to two paid TTS calls per segment rather than one; previously only overrunning segments paid twice. Hopelessly mismatched segments (see step 3) are the exception: they always cap at one TTS call, since a second, clamped-rate call would not meaningfully help.
...
3. `dub.py:synthesise_track` iterates segments, calls `_synthesise_fitted` per segment: synthesise at speed 1.0, measure with `ffprobe`, and if the clip's duration drifts from its slot by more than roughly 1% (`round(rate, 2) != 1.0`, over or under), compute a clamped rate via `fit_rate` and re-synthesise once at that rate — both overruns and underruns are re-synthesised, not just overruns. This retry is skipped for a hopeless slot — non-positive duration, or an ideal rate past `_HOPELESS_RATE = 2.0` — where the clip is instead placed unfitted at 1.0x with a single warning; a ±15% clamp would be noise against a mismatch that large, so it is not worth a second paid TTS call. Segments that still overrun at the clamp bound after the retry, and hopeless segments (which overrun by construction, having never been fitted), are logged and counted together; an underrun is never counted, it just leaves trailing silence in its slot.
...
- Timing drift is handled by one bounded strategy (length-budgeted translation → clamped rate nudge, both directions → log-and-overrun on failure), deliberately not made pluggable. Both overruns and underruns are re-synthesised at the clamped rate; only a still-overrunning clip after the retry is logged and let overrun into the following silence. A hopelessly mismatched slot (non-positive duration, or an ideal rate past `_HOPELESS_RATE = 2.0`) skips the clamped-rate retry outright, since a ±15% nudge cannot meaningfully help — the clip is placed unfitted at 1.0x, logged once, and counted as an overrun without a second TTS call.
