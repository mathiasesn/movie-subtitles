...
movie_subtitles/
  ...
  dub.py                  # scene-anchored TTS synthesis (fit_rate, synthesise_track):
                           ...
                           # default 8), each batch resolved fail-fast by `_resolve()`
                           # (cancels the still-queued tail on the first exception,
                           # re-raises it); results are collected by submission index so
  ...
  providers/
    ...
    fallback.py             # FallbackAlign: composes AlignmentProviders into a
                            ...
                            # aligner concurrently across the worker pool)
...
| `--dub-workers` | `8` | Max concurrent TTS/alignment calls during `--dub` synthesis (`synthesise_track`'s `max_workers`, one bounded `ThreadPoolExecutor`); `1` restores the old serial behaviour — an escape hatch for vendor rate limits |
