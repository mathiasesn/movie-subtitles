...
movie_subtitles/
  ...
  providers/
    ...
    fallback.py             # FallbackAlign: composes AlignmentProviders into a
                            ...
                            # one thread makes it -- a broken tier costs one failing call
                            # for the whole run, not one per worker -- and settling on a
                            # tier is logged exactly once per run, not once per degrade
